...
import static code0.land_location.AdminAdd.cordinate;
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == PLACE_PICKER_REQUEST) {
            if (resultCode == RESULT_OK) {
                ...
                //cordinate=String.valueOf(place.getLatLng());
                LatLng  latLng = place.getLatLng();
                Double latlat= latLng.latitude;
                Double longlong= latLng.longitude;
                cordinate = String.valueOf(latlat)+","+String.valueOf(longlong);

                new AlertDialog.Builder(PlaceViewer.this)
                        .setMessage(toastMsg +String.valueOf(cordinate))
                        .setPositiveButton("Okay", new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialogInterface, int i) {
                                PlaceViewer.this.finish();
                               // startActivity(new Intent(PlaceViewer.this, AdminAdd.class));
